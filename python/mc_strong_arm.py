...
Vcm = 0.9131763339042664
time, Vsc_mc = OutputParser(CktGraph()).tran(f'{NETLIST_NAME}_Vsc_mc_pex_prelayout')
_, Voutn_mc = OutputParser(CktGraph()).tran(f'{NETLIST_NAME}_Voutn_mc_pex_prelayout')
_, Voutp_mc = OutputParser(CktGraph()).tran(f'{NETLIST_NAME}_Voutp_mc_pex_prelayout')
...
op_results = results.dcop(f'{NETLIST_NAME}_op_opt_final') # no mismatch variation, no process vatiation, used for Vos prediction only
...
plt.plot(bins_centers, ym_r, c='r', label='$\mathbf{\mu=-0.65mV}$ \n $\mathbf{\sigma={6.13mV}}$')
plt.hist(Voffset_r_mc*1e3, bins)
plt.xlabel('$\mathbf{V_{offset}}$ (mV)', fontweight='bold', fontsize=14)
...
# plt.savefig(f'./pics/mc_rise_strong_arm_comp.pdf', format='pdf', bbox_inches='tight')
# plt.savefig(f'./pics/mc_rise_strong_arm_comp.png', format='png', bbox_inches='tight')
...
plt.plot(bins_centers, ym_f, c='r', label='$\mathbf{\mu=-0.68mV}$ \n $\mathbf{\sigma={6.04mV}}$')
...
ax.spines['right'].set_linewidth(2)
ax.spines['top'].set_linewidth(2)
# plt.savefig(f'./pics/mc_fall_strong_arm_comp.pdf', format='pdf', bbox_inches='tight')
# plt.savefig(f'./pics/mc_fall_strong_arm_comp.png', format='png', bbox_inches='tight')

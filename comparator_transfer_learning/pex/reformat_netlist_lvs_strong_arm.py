...
.subckt strong_arm_comparator Vin_p Vin_n CLK VDD Vout_n Vout_p VSS
...
device_info_dir =  '/autofs/fs1.ece/fs1.eecg.tcc/lizongh2/sky130_comparator/laygo2_workspace_sky130/laygo2_example/strong_arm/strong_arm_device_size.yaml'
with open(device_info_dir, 'r') as stream:
    device_info = yaml.safe_load(stream) 

# load the schematic netlist from Xschem
with open(f'/autofs/fs1.ece/fs1.eecg.tcc/lizongh2/sky130_comparator/comparator_transfer_learning/pex/simulations/strong_arm_comp.spice', "r") as netlist:
    lines = netlist.readlines()

# generate the LVS compatible netlist for netegn
with open(f'{STRONG_ARM_NETGEN_DIR}/schematic/strong_arm_comparator_lvs.spice', "w") as netlist:
    for line in lines:
        ...
        for i in range(0,len(line)): 
            ...
            if line[i:i+5] == 'W_M6 ': # there is a space here in the end
                line = line.replace('W_M6', f'{device_info["M5"]["W_discrete"]}')
                for j in range(0,len(line)):
                    if line[j:j+4] == 'nf=1':
                        line = line.replace('nf=1', f'nf={device_info["M5"]["nf"]}')
            ...
            if line[i:i+5] == 'W_M8 ': # there is a space here in the end
                line = line.replace('W_M8', f'{device_info["M8"]["W_discrete"]}')
                for j in range(0,len(line)):
                    if line[j:j+4] == 'nf=1':
                        line = line.replace('nf=1', f'nf={device_info["M8"]["nf"]}')
            if line[i:i+5] == 'W_M9 ': # there is a space here in the end
                line = line.replace('W_M9', f'{device_info["M8"]["W_discrete"]}')
                for j in range(0,len(line)):
                    if line[j:j+4] == 'nf=1':
                        line = line.replace('nf=1', f'nf={device_info["M8"]["nf"]}')

...
class DoubleTailCompEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def _initialize_simulation(self):
        ...
        np.array([1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1.1]) 
        # self.Vcm: initial input common-mode voltage, from Nauta's ISSCC paper
    ...
    def _do_simulation(self, action: np.array):
        ...
        # We enforce W_M6 to be at least larger than W_M7 to surpress regeneration
        # and also improve the offset performance
        if W_M6 < 2 * W_M7:
            W_M6 = min(2*W_M7, 10e-6)
    ...
    def _get_info(self):
        ...
        time, Vclk = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Vclk')        
        _, Voutp = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Voutp')
        _, Voutn = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Voutn')
        _, Vdip = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Vdip')
        _, Vdin = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Vdin')
        time = np.array(time)
        ...
        # 1.5 is just a scaling factor to compensate the error between the prediction model and MC simulation results, 
        # it is emperical.
        # print(self.total_offset)
        self.Vos = 1.5 * self.total_offset
        if self.Vout_score < 0 or self.Vout_reset_score < 0 or self.Vdi_reset_score < 0: 
            self.Vos_score = -1
        else:
        ...
        ''' Energy consumption '''
        _, self.Ibias = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Ibias')
        ...
        ''' Kickback noise '''
        _, Vn_kn = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Vn_kn')
        _, Vp_kn = self.sim_results.tran(file_name=f'{NETLIST_NAME}_Vp_kn')
    ...
    def input_offset(self, W_M1, L_M1,
        ...
        def random_mismatch(W, L, par=1):
            ...
            A_Vth = 0.007148
            sigma_Vth = 0.7071 * A_Vth * 1e-6 / p_sqrtarea
            
            # mismatch from charge mobility multiplier "mulu0", nfet 3.3V
            A_k = 0.007008
            sigma_k = 0.7071 * A_k * 1e-6 / p_sqrtarea
            
            return sigma_Vth, sigma_k

        sigma_Vth_M1, sigma_k_M1 = random_mismatch(W_M1, L_M1)
        sigma_Vth_M2 = sigma_Vth_M1
        sigma_k_M2 = sigma_k_M1
        ...
        sigma_z = np.sqrt(sigma_k_M1**2 + sigma_k_M2**2) 
